...
var NOTIFY = ['kacper.trzaskalski@itf.pl', 'adam.bondarzewski@itf.pl'];
 
// Yesturday downloads will be saved in DRIVE_DIR_DAILY/2016-02-20/AD_PERFORMANCE_REPORT_830-454-0281.csv 
var DRIVE_DIR_DAILY_NAME = 'reports';

var FNAME_ACCOUNTS_LIST = 'accounts_list.json';

var PARALLEL_THREADS = 50;
var ROWS_MAX = 100000;

var start_date_report = get_7_days_ago_str();

var end_date_report = get_yesturday_str();

var time_period = start_date_report + ',' + end_date_report;

var CONFIG = {

  // Default date range over which statistics fields are retrieved.
  DEFAULT_DATE_RANGE: time_period,
...
*/
  ...
  REPORTS: [{NAME: 'AD_PERFORMANCE_REPORT',
                   CONDITIONS: 'WHERE Impressions > 0 AND Status IN [ENABLED, PAUSED, DISABLED]',
                   FIELDS: {
...
Logger.log(time_period);
  ...
  if (!day_dir_locked()) {
    ...
    try {
      create_accounts_list_file();
      download_yesturday_raports();
    } catch (err) {
      Logger.log(err);
      if (!day_dir_locked()) {
        delete_lock();
